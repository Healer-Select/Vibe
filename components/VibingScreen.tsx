...
import { Contact, UserProfile, VibeCategory, VibeAction, VibePattern, VibeSignal } from '../types';
import { ChevronLeft, Heart, Plus, X, CircleDot, MessageCircle, Activity, PenTool, Wind, Grid3X3, Play, Pause, Square } from 'lucide-react';
...
const VibingScreen: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    ...
    return () => {
        window.clearInterval(interval);
        // Important: Do not send stop on unmount, allows persistent background pulse if intended
    };
  ...
  const toggleBreathe = () => {
    if (isBreatheActive) {
        ...
        onSendVibe('breathe', 'invite', { variant: breatheVariant }); // Sync start
  ...
  const handleEnd = () => {
    ...
    if (duration > 500) {
      setStatus('sending');
      onSendVibe('touch', 'data', { touchType: 'hold', duration }).finally(() => setStatus('idle'));
      ...
      tapTimer.current = window.setTimeout(() => {
        ...
        setStatus('sending');
        onSendVibe('touch', 'data', { touchType: 'tap', count: finalCount }).finally(() => {
          setStatus('idle');
          setTapCount(0);
  ...
  const handleMatrixTileClick = (index: number) => {
      ...
      triggerHaptic(50);
      onSendVibe('matrix', 'select', { selectionIndex: index });
  ...
  const handleDrumEnd = () => {
    ...
    const vibe = duration > 250 ? Math.min(duration, 3000) : 100;
    triggerHaptic(vibe);
    setRecordingData(prev => [...prev, vibe, 150]);
  };
      ...
      <div key={pattern.id} className={`relative group w-16 h-16 ${status === 'sending' || status === 'recording' ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        <button
          onClick={() => {
            ...
            onSendVibe('touch', 'data', { touchType: 'pattern', patternData: pattern.data, patternEmoji: pattern.emoji, patternName: pattern.name }).finally(() => setStatus('idle'));
          }}
  ...
  const renderContent = () => {
    if (mode === 'heartbeat') {
        return (
            <div className="flex-1 flex flex-col items-center justify-center animate-in zoom-in duration-300 relative">
                 <button onClick={() => { setIsHeartbeatActive(false); onSendVibe('heartbeat', 'stop', {}); setMode('default'); }} className="absolute top-4 right-4 p-2 glass-button rounded-full text-white/60 hover:text-white">
                    <X size={24} />
                 </button>
                <p className="text-orange-200/60 uppercase tracking-widest text-xs font-bold mb-12">
                    {isHeartbeatActive ? 'Sending Pulse...' : 'Heartbeat Ready'}
                </p>
                <button 
            ...
            <div className="fixed inset-0 z-50 bg-black animate-in fade-in duration-500">
                ...
                />
                 ...
                 <div className="absolute bottom-24 left-0 right-0 flex items-center justify-center gap-4">
                    {['#e879f9', '#22d3ee', '#f472b6', '#a78bfa'].map(c => (
    ...
    if (mode === 'breathe') {
        return (
            <div className="flex-1 flex flex-col items-center justify-center animate-in fade-in duration-1000 relative">
                 <button onClick={() => { setMode('default'); setIsBreatheActive(false); onSendVibe('breathe', 'stop', {}); }} className="absolute top-4 right-4 p-2 glass-button rounded-full text-white/60 hover:text-white">
                    <X size={24} />
                 </button>
                 ...
                 <div className="relative flex items-center justify-center mb-16">
                     ...
                     <div className={`w-48 h-48 border-2 ${isBreatheActive ? 'border-sky-300/40 shadow-[0_0_60px_rgba(125,211,252,0.2)]' : 'border-white/10'} rounded-full flex items-center justify-center transition-all ease-in-out backdrop-blur-sm`} style={{ transform: (breathePhase === 'in' && isBreatheActive) ? 'scale(1.5)' : 'scale(1)', transitionDuration: '4s' }}>
    ...
    if (mode === 'matrix') {
        return (
            <div className="flex-1 flex flex-col items-center justify-center animate-in slide-in-from-bottom duration-500 relative w-full h-full p-4">
                 <button onClick={() => { setMode('default'); onSendVibe('matrix', 'reset', {}); }} className="absolute top-2 right-2 p-2 glass-button rounded-full text-white/60 hover:text-white z-10">
                    <X size={24} />
                 </button>
                 {/* Simplified for brevity - Logic mostly handled in state/effects */}
                 {matrixState === 'briefing' ? (
                     <div className="text-center space-y-4">
                         <h2 className="text-2xl font-outfit font-bold text-white">Telepathy</h2>
                         <button onClick={startMatrixGame} className="bg-cyan-100/90 text-cyan-900 py-3 px-8 rounded-full font-bold">Start</button>
                     </div>
                 ) : (
                     <div className="grid gap-2 w-full max-w-sm aspect-[3/5]" style={{ gridTemplateColumns: `repeat(3, 1fr)` }}>
                         {Array.from({ length: 9 }).map((_, i) => (
                            <button key={i} onClick={() => handleMatrixTileClick(i)} className={`rounded-xl border border-white/10 ${myMatrixSelection === i ? 'bg-cyan-500' : 'bg-white/5'}`} />
                         ))}
                     </div>
                 )}
            </div>
    ...
    return (
        <div className="flex-1 flex flex-row items-center justify-between relative overflow-hidden">
            <button onClick={onBack} className="absolute top-4 left-4 p-4 glass-button rounded-full text-white/60 hover:text-white z-50">
                <ChevronLeft size={24} />
            </button>
            ...
            <div className="absolute bottom-6 inset-x-0 flex justify-center gap-3">
                ...
                <button onClick={onOpenChat} className="w-14 h-14 glass-button rounded-2xl flex items-center justify-center"><MessageCircle size={20} /></button>
            </div>
            
            {showRecorder && (
                <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-2xl flex flex-col p-8 items-center justify-center">
                     <div onMouseDown={handleDrumStart} onMouseUp={handleDrumEnd} onTouchStart={handleDrumStart} onTouchEnd={handleDrumEnd} className="w-52 h-52 rounded-[4rem] glass-button flex items-center justify-center">
                           <CircleDot size={80} className="text-white/40" />
                     </div>
                     <button onClick={() => { 
                             if (recordingData.length > 0) {
                                 onSavePattern({ id: generateId(), name: 'Pattern', emoji: '✨', data: recordingData });
                                 setRecordingData([]); setShowRecorder(false); setStatus('idle');
                             }
                        }} className="mt-8 py-4 px-12 bg-pink-600 text-white font-bold rounded-2xl">Save</button>
